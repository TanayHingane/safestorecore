...
export const Sidebar: React.FC = () => {
  const { totalStorageUsed, currentView, changeView, emptyTrash } = useFileSystem();
  ...
  const NavButton = ({ view, icon: Icon, label }: { view: SidebarViewMode, icon: any, label: string }) => {
  ...
  return (
    <div className="w-64 bg-slate-50 border-r border-slate-200 flex flex-col h-full flex-shrink-0">
      <div className="p-6 flex items-center space-x-3">
        ...
        <span className="text-xl font-bold text-slate-800 tracking-tight">GeminiDrive</span>
      </div>
      ...
      <div className="p-6 border-t border-slate-200">
        ...
        <div className="h-2 w-full bg-slate-200 rounded-full overflow-hidden">
            <div className="h-full bg-indigo-600 transition-all duration-500" style={{ width: `${usedPercentage}%` }}></div>
        </div>
        <p className="mt-2 text-xs text-slate-500">{formatSize(totalStorageUsed)} of 5 GB used</p>
      </div>
    </div>
  );
};