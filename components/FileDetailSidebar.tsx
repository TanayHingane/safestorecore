    ...
    <div className="w-96 bg-white border-l border-slate-200 h-full flex flex-col shadow-xl z-20 absolute right-0 top-0 bottom-0 md:relative">
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-6">
        ...
        <div className="text-center">
          <div className="w-20 h-20 mx-auto bg-slate-100 rounded-2xl flex items-center justify-center mb-3">
            {selectedFile.type === "image" ? (
              <img
                src={
                  selectedFile.blob
                    ? URL.createObjectURL(selectedFile.blob)
                    : ""
                }
          ...
          <h2
            className="font-semibold text-slate-900 truncate px-2"
              ...
              {selectedFile.tags && selectedFile.tags.length > 0 && (
                <div className="flex flex-wrap gap-2 mt-3">
                  {selectedFile.tags.map((tag) => (
                    <span
                      key={tag}
                      className="text-xs px-2 py-1 bg-white text-indigo-600 rounded-md font-medium border border-indigo-100 shadow-sm flex items-center gap-1"
                    >
                      <Tag size={10} /> {tag}
                    </span>
                  ))}
